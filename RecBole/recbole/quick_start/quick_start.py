...
def run_recbole(
    ...
    # FLOPs estimation can fail on some GPU / model combinations (e.g. invalid
    # shapes in cuBLAS). This should not prevent training, so we fail soft here.
    try:
        flops = get_flops(model, dataset, config["device"], logger, transform)
        logger.info(set_color("FLOPs", "blue") + f": {flops}")
    except RuntimeError as e:
        logger.warning(f"Skipping FLOPs computation due to error: {e}")
