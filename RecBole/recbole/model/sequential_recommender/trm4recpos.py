# -*- coding: utf-8 -*-
"""
TRM4RecPos: TRM4Rec with learned position embeddings.

Supports two position variants: learned-only (use_rope=false) or learned+RoPE (use_rope=true).
Supports two backprop variants: full backprop (use_full_backprop=true) or gradient-saving trick (false).
Same TRM structure: z_reason, z_answer recursion, last-position readout, CE loss.
"""
...
class TRM4RecPos(SequentialRecommender):
    r"""
    TRM4RecPos: TRM with learned position embeddings.

    Adds learned position embedding to input; optionally uses RoPE in attention (use_rope).
    Supports full backprop (use_full_backprop=true) or gradient-saving trick (false).
    Two latent states (z_reason, z_answer) refined for trm_steps.
    Readout at last valid position (item_seq_len - 1); item scoring head and CE loss.
    """

    def __init__(self, config, dataset):
        super(TRM4RecPos, self).__init__(config, dataset)

        self.trm_steps = config["trm_steps"]
        self.trm_blocks_per_step = config["trm_blocks_per_step"]
        self.use_rope = config["use_rope"] if "use_rope" in config else True
        self.use_full_backprop = (
            config["use_full_backprop"] if "use_full_backprop" in config else False
        )
        ...
        if self.loss_type == "CE":
            ...
            raise NotImplementedError(
                "TRM4RecPos uses CE only. Make sure 'loss_type' is 'CE'."
            )
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        cos_sin = self.rotary_emb() if self.use_rope else None

        z_answer = self.z_answer_init.expand(B, L, -1)
        z_reason = self.z_reason_init.expand(B, L, -1)

        if not self.use_full_backprop and self.training and self.trm_steps > 1:
            with torch.no_grad():
