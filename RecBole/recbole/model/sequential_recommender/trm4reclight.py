# -*- coding: utf-8 -*-
"""
TRM4RecLight: TRM4Rec with LightSANs-style low-rank attention in each block.
Same recursion, readout, and loss as TRM4Rec; uses TRMBlockLight (item-to-interest aggregation).
Does not modify trm4rec.py or trm_layers.py.
"""
...
from recbole.model.sequential_recommender.trm_layers import RotaryEmbedding
from recbole.model.sequential_recommender.trm_layers_light import TRMBlockLight


class TRM4RecLight(SequentialRecommender):
    r"""
    TRM4Rec with low-rank (LightSANs-style) attention per block.
    Same structure as TRM4Rec: z_reason/z_answer recursion, last-position readout, CE loss.
    Uses TRMBlockLight (item-to-interest aggregation) instead of full self-attention.
    """

    def __init__(self, config, dataset):
        super(TRM4RecLight, self).__init__(config, dataset)
        ...
        if self.loss_type == "CE":
            ...
            raise NotImplementedError(
                "TRM4RecLight uses CE only. Make sure 'loss_type' is 'CE'."
            )
