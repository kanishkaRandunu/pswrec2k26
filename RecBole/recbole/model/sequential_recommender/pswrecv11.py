...
"""
pswrecv11: V11 + Behavioral Rotary Attention (B-RoPE).

This variant keeps the PSWRecV5 architecture (filterbank + FFN) but replaces
the phase-biased attention with a behavioral rotary attention mechanism that:
  - Rotates Q and K using behavioral phase φ from the filterbank
  - Leaves V untouched (preserves semantic payload)
  - Embeds cos(Δφ) directly into the geometry of the attention space
"""
...
class LocalPhaseFilterBankV5(nn.Module):
    r"""LocalPhaseFilterBankV5

    A lightweight, learnable, multi-band filterbank over the sequence axis.
    Same as V4H; quadrature pair (U_s, V_s) and phase/mag output.
    """
...
def rotate_half(x: torch.Tensor) -> torch.Tensor:
    """Rotate last dimension by 90 degrees in complex plane.

    Splits the last dim into two halves (real, imag) and applies:
      (x1, x2) -> (-x2, x1)
    """
...
class BehavioralRotaryAttentionV11(nn.Module):
    """Behavioral Rotary Attention (B-RoPE) over Q/K using behavioral phase φ.

    - Q, K are rotated by phase angles derived from the filterbank.
    - V is left untouched, preserving semantic payload.
    """
    ...
    def __init__(
        ...
        # Project bands -> head_dim//2 phase angles, then duplicate to full head_dim
        self.phase_proj = nn.Linear(n_bands, self.head_dim // 2)
    ...
    def forward(
        ...
        # Behavioral phase projection
        theta_half = self.phase_proj(phi)  # [B, L, d/2]
        theta = torch.cat((theta_half, theta_half), dim=-1)  # [B, L, d]
        theta = theta.unsqueeze(1)  # [B, 1, L, d]

        cos_theta = torch.cos(theta)
        sin_theta = torch.sin(theta)

        # Apply rotary to Q and K only
        q_rot = (q * cos_theta) + (rotate_half(q) * sin_theta)
        k_rot = (k * cos_theta) + (rotate_half(k) * sin_theta)

        # Semantic attention
        attn_scores = torch.matmul(q_rot, k_rot.transpose(-1, -2))  # [B, H, L, L]
...
class FeedForwardV11(nn.Module):
    ...
    def __init__(
        ...
        if hidden_act not in act_map:
            raise ValueError(f"Unsupported hidden_act '{hidden_act}' in pswrecv11.")
...
class PSWBlockV11(nn.Module):
    """Single transformer-style layer: BehavioralRotaryAttentionV11 + FFN."""
    ...
    def __init__(
        ...
        super().__init__()
        self.attn = BehavioralRotaryAttentionV11(
            n_heads=n_heads,
...
class PSWEncoderV11(nn.Module):
    """Stack of PSWBlockV11 layers."""
...
class pswrecv11(SequentialRecommender):
    r"""pswrecv11: PSWRecV5 backbone + Behavioral Rotary Attention (B-RoPE)."""
    ...
    def __init__(self, config, dataset):
        ...
        # Phase-related scalars kept for config compatibility, not used directly.
    ...
    def forward(self, item_seq: torch.Tensor, item_seq_len: torch.Tensor) -> torch.Tensor:
        ...
        # Optional phase-smoothness regularisation stays on cos/sin
        if self.phase_aux:
        ...
        # Reconstruct φ and reshape to [B, L, n_bands] for B-RoPE
        phi = torch.atan2(sin_phi, cos_phi)          # [B, n_bands, L]
        phi_pl = phi.permute(0, 2, 1).contiguous()   # [B, L, n_bands]
