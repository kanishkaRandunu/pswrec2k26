# -*- coding: utf-8 -*-
"""
TRM4RecFullBP: TRM4Rec with full backpropagation in all recursive steps.

Same building blocks as TRM4Rec (RMSNorm, SwiGLU, RoPE, TRM attention), but no
gradient-saving trick: all trm_steps run with gradients during training.
...
class TRM4RecFullBP(SequentialRecommender):
    r"""
    TRM4RecFullBP: TRM4Rec with full backpropagation in all recursive steps.

    Uses TRM blocks (RMSNorm, SwiGLU, RoPE, TRM attention), no learned position embedding.
    Two latent states (z_reason, z_answer) refined for trm_steps; all steps run with
    gradients (no gradient-saving trick).
    Readout at last valid position (item_seq_len - 1); item scoring head and CE loss.
    """

    def __init__(self, config, dataset):
        super(TRM4RecFullBP, self).__init__(config, dataset)
        ...
        if self.loss_type == "CE":
            ...
            raise NotImplementedError(
                "TRM4RecFullBP uses CE only. Make sure 'loss_type' is 'CE'."
            )
