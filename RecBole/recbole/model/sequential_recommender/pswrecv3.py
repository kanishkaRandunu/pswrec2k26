...
"""
pswrecv3: Rhythmic-residual PSWRec variant.

This model keeps the PSW-style architecture (causal self-attention + local
time–frequency filterbank + phase bias), but:

- Uses linear magnitude weighting (average energy (Ai + Aj)/2) in the phase
  bias instead of squared Ai * Aj, so the rhythmic signal is softened rather
  than hard-gated.
- Uses updated band dilations for better local–global coverage (configured via
  band_dilations, typically [1, 2, 4, 8] on Beauty).
- Restores stronger regularization (dropout=0.5, inner_size=256, higher
  phase_aux_weight) to favor generalization over memorization.
"""
...
class LocalPhaseFilterBankV3(nn.Module):
    r"""LocalPhaseFilterBankV3
...
class PhaseSyncAttentionV3(nn.Module):
    r"""Multi-head self-attention with magnitude-weighted phase residuals.

    For each head h, we compute a band-weighted phase term scaled by the
    average magnitude of the pair:

        Bias_ij^h = beta_h * ( (A_i^h + A_j^h) / 2 ) *
                    Σ_s w_{h,s} cos(Φ_{i,s} - Φ_{j,s})

    where A_i^h and A_j^h are band-weighted magnitudes at positions i, j.
    """
    ...
    def __init__(
        ...
        # Per-head phase bias scale (exponentiated to keep beta > 0).
        self.phase_bias_scale = nn.Parameter(
            torch.full((n_heads,), float(phase_bias_scale))
        )

        # Head-specific quadratic bias over bands: short-term / long-term split.
        with torch.no_grad():
    ...
    def forward(
        ...
        # Band-weighted cos/sin features per head: [B, H, L, S]
...
class FeedForwardV3(nn.Module):
    ...
    def __init__(
        ...
        if hidden_act not in act_map:
            raise ValueError(f"Unsupported hidden_act '{hidden_act}' in pswrecv3.")
...
class PSWBlockV3(nn.Module):
    """Single transformer-style layer: PhaseSyncAttentionV3 + FFN."""
    ...
    def __init__(
        ...
        super().__init__()
        self.attn = PhaseSyncAttentionV3(
            n_heads=n_heads,
...
class PSWEncoderV3(nn.Module):
    """Stack of PSWBlockV3 layers."""
...
class pswrecv3(SequentialRecommender):
    r"""pswrecv3: Rhythmic-residual PSWRec variant."""
    ...
    def __init__(self, config, dataset):
        ...
        if "band_dilations" in config:
            ...
            # V3 default dilations favor multi-scale coverage.
        ...
        self.phase_filter = LocalPhaseFilterBankV3(
            hidden_size=self.hidden_size,
            kernel_sizes=band_kernel_sizes,
            dilations=band_dilations,
        )
        self.encoder = PSWEncoderV3(
    ...
    def forward(self, item_seq: torch.Tensor, item_seq_len: torch.Tensor) -> torch.Tensor:
        ...
        # Phase smoothness auxiliary loss.
        if self.phase_aux:
