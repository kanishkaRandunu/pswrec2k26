...
"""
pswrecv5: V5 + Post-softmax phase gating/reweighting (richer use of phase).

Same as pswrecv4h (harmonic-mean phase bias) but after softmax we apply a
phase-based gate to the attention weights: gate = sigmoid(phase_gate_scale * phase_scores),
then attn_probs_gated = attn_probs * gate and renormalize. This downweights
pairs with very different phase beyond the additive bias.
"""
...
class PhaseSyncAttentionV5(nn.Module):
    ...
    Same as V4H (additive phase bias with harmonic-mean mag gate) plus: after softmax,
    gate attention weights by phase similarity and renormalize.
    """
...
class FeedForwardV5(nn.Module):
    ...
    def __init__(
        ...
        if hidden_act not in act_map:
            raise ValueError(f"Unsupported hidden_act '{hidden_act}' in pswrecv5.")
...
class pswrecv5(SequentialRecommender):
    r"""pswrecv5: V4H + post-softmax phase gating/reweighting.

    Same as pswrecv4h but after softmax, attention weights are gated by phase
    similarity and renormalized (richer use of phase).
    """
    ...
    def __init__(self, config, dataset):
        ...
        self.phase_gate_scale = (
            float(config["phase_gate_scale"])
            if "phase_gate_scale" in config
            else 1.0
        )
