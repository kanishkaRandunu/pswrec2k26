...
"""
pswrecv4g: V4 + Gabor Kickstart.

Same as pswrecv4 (geometric-mean gated PSWRec) but the LocalPhaseFilterBank
is initialized and parameterized by a Gabor function:

    g(x, mu, sigma, lambda, theta) = exp(-(x-mu)^2/(2*sigma^2)) * cos(2*pi*(x-mu)/lambda + theta)

Real kernel: theta=0 (cosine). Imag kernel: theta=pi/2 (sine) for quadrature.
Per band, mu, sigma, and lambda are learnable and start at physically ideal values
(center, spread, wavelength); the kernel is computed from them at each forward.
"""
...
class LocalPhaseFilterBankV4G(nn.Module):
    r"""LocalPhaseFilterBankV4G: Gabor-parameterized filterbank.

    Per band s with kernel size k_s, we store learnable mu_raw_s, log_sigma_s, log_lambda_s
    (initialized so mu = (k-1)/2, sigma = k/4, lambda = k/2). At forward we compute
    the Gabor kernel from these and apply depthwise conv (real and imag).
    """
    ...
    def __init__(
        ...
        # Learnable Gabor parameters per band: [n_bands, 3] = (mu_raw, log_sigma, log_lambda)
        # mu = (k-1)*sigmoid(mu_raw), sigma = softplus(log_sigma)+eps, lambda = softplus(log_lambda)+eps
        params_list = []
        for s, k in enumerate(kernel_sizes):
            mu_center = (k - 1) / 2.0
            sigma_ideal = k / 4.0
            lambda_ideal = max(k / 2.0, 2.0)
            # mu_raw init so sigmoid(mu_raw)=0.5 => mu_raw=0
            mu_raw_init = 0.0
            log_sigma_init = _inv_softplus(sigma_ideal)
            log_lambda_init = _inv_softplus(lambda_ideal)
            params_list.append([mu_raw_init, log_sigma_init, log_lambda_init])
        gabor_params = torch.tensor(params_list, dtype=torch.float32)  # [n_bands, 3]
        self.gabor_params = nn.Parameter(gabor_params)

        self.mag_eps = 1e-8
        self.mag_tau = 1e-3

    def _gabor_kernels(self, band_idx: int, k: int, d: int, device: torch.device, dtype: torch.dtype) -> Tuple[torch.Tensor, torch.Tensor]:
        """Compute real and imag 1D Gabor kernels for band band_idx (length k)."""
        mu_raw = self.gabor_params[band_idx, 0]
        log_sigma = self.gabor_params[band_idx, 1]
        log_lambda = self.gabor_params[band_idx, 2]

        mu = (k - 1) * torch.sigmoid(mu_raw)
        sigma = F.softplus(log_sigma) + 1e-6
        lam = F.softplus(log_lambda) + 1e-6

        x = torch.arange(k, device=device, dtype=dtype)
        envelope = torch.exp(-((x - mu) ** 2) / (2 * sigma ** 2))
        phase_arg = 2 * math.pi * (x - mu) / lam

        real_k = envelope * torch.cos(phase_arg)
        imag_k = envelope * torch.cos(phase_arg + math.pi / 2.0)

        # L2 normalize so conv output scale is stable
        real_norm = torch.sqrt((real_k ** 2).sum() + 1e-8)
        imag_norm = torch.sqrt((imag_k ** 2).sum() + 1e-8)
        real_k = real_k / real_norm
        imag_k = imag_k / imag_norm

        return real_k, imag_k
    ...
    def forward(
        ...
        for s, (pad, d) in enumerate(zip(self.band_pads, self.dilations)):
            k = self.kernel_sizes[s]
            device = x_t.device
            dtype = x_t.dtype

            real_1d, imag_1d = self._gabor_kernels(s, k, d, device, dtype)
            # [k] -> [1, 1, k] -> [hidden_size, 1, k] for depthwise conv
            real_weight = real_1d.view(1, 1, -1).expand(self.hidden_size, 1, k)
            imag_weight = imag_1d.view(1, 1, -1).expand(self.hidden_size, 1, k)

            x_padded = F.pad(x_t, (pad, 0))
            u = F.conv1d(x_padded, real_weight, groups=self.hidden_size, dilation=d)
            v = F.conv1d(x_padded, imag_weight, groups=self.hidden_size, dilation=d)
...
class PhaseSyncAttentionV4G(nn.Module):
    r"""Multi-head self-attention with geometric-mean phase gating (same as V4)."""
    ...
    def forward(
        ...
        geo_mag = torch.sqrt(mag_i * mag_j + 1e-8)
...
class FeedForwardV4G(nn.Module):
    ...
    def __init__(
        ...
        if hidden_act not in act_map:
            raise ValueError(f"Unsupported hidden_act '{hidden_act}' in pswrecv4g.")
...
class PSWBlockV4G(nn.Module):
    """Single transformer-style layer: PhaseSyncAttentionV4G + FFN."""
    ...
    def __init__(
        ...
        super().__init__()
        self.attn = PhaseSyncAttentionV4G(
            n_heads=n_heads,
...
class PSWEncoderV4G(nn.Module):
    """Stack of PSWBlockV4G layers."""
...
class pswrecv4g(SequentialRecommender):
    r"""pswrecv4g: V4 + Gabor kickstart.

    Same as pswrecv4 but the filterbank is Gabor-parameterized (learnable mu, sigma, lambda per band).
    """
    ...
    def __init__(self, config, dataset):
        ...
        self.phase_filter = LocalPhaseFilterBankV4G(
            hidden_size=self.hidden_size,
            kernel_sizes=band_kernel_sizes,
            dilations=band_dilations,
        )
        self.encoder = PSWEncoderV4G(
