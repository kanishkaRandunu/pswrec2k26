...
"""
pswrecv2: Magnitude-gated, Gabor-initialized PSWRec variant.

This model keeps the overall PSWRec design (causal self-attention over item
sequence + local time–frequency filterbank + phase-synchrony bias), but:

- Uses a magnitude-gated synchronization term: only high-energy bands can
  meaningfully increase attention via phase alignment.
- Initializes the filterbank with Gabor-style sine/cosine kernels to give
  each band a sensible starting frequency response.

We keep hidden_size=64 by design so comparisons to SASRec/LightSANs/PSWRec
remain fair on Amazon Beauty.
"""
...
class LocalPhaseFilterBankV2(nn.Module):
    r"""LocalPhaseFilterBankV2

    Causal, multi-band depthwise filterbank with explicit magnitude output.

    - Input:  X in R^{B x L x D}
    - For each band s, apply depthwise Conv1d over sequence (causal via
      left-padding) to get quadrature pair (U_s, V_s).
    - Magnitude A_s and phase Φ_s are derived per band and position.
    """
    ...
    def forward(
        ...
        Returns:
            ...
            A:       [B, S, L]  magnitude per band and position
        """
        B, L, D = x.size()
        ...
        for pad, conv_r, conv_i in zip(
            ...
            # Magnitude and band-wise aggregation.
            mag = torch.sqrt(u * u + v * v + self.mag_eps)  # [B, D, L]
            mag_band = mag.mean(dim=1)  # [B, L]

            u_band = u.mean(dim=1)  # [B, L]
            v_band = v.mean(dim=1)  # [B, L]
            us.append(u_band)
            vs.append(v_band)
            mags.append(mag_band)

        U = torch.stack(us, dim=1)  # [B, S, L]
        V = torch.stack(vs, dim=1)  # [B, S, L]
        A = torch.stack(mags, dim=1)  # [B, S, L]

        phi = torch.atan2(V, U + self.mag_eps)  # [B, S, L]
        cos_phi = torch.cos(phi)
        sin_phi = torch.sin(phi)

        return cos_phi, sin_phi, A


class PhaseSyncAttentionV2(nn.Module):
    r"""Multi-head self-attention with magnitude-gated phase-synchrony bias.

    For each head h, we compute a band-weighted, magnitude-gated phase term:

        score_ij^h = (q_i^h · k_j^h) / sqrt(d_h)
                     + beta_h * Σ_s w_{h,s} mag_{i,s} mag_{j,s} cos(Φ_{i,s} - Φ_{j,s})

    This is implemented efficiently using dot products over band features with
    magnitude and band weights folded into the feature vectors.
    """
    ...
    def __init__(
        ...
        # Per-head phase bias scale (we'll exponentiate to keep beta > 0).
    ...
    def forward(
        ...
        r"""Args:
        hidden_states: [B, L, D]
        attention_mask: [B, 1, 1, L] with 0 for keep, -10000 for masked
        cos_phi, sin_phi, mag: [B, S, L]
        """
        ...
        # Phase-synchrony bias with magnitude gating.
        # [B, S, L] -> [B, L, S]
        ...
        # Build per-head band-weighted, magnitude-weighted phase features.
        # shape: [B, H, L, S]
        cos_feat = (
            cos_pl.unsqueeze(1)
            * mag_pl.unsqueeze(1)
            * band_weights_sqrt.view(1, self.n_heads, 1, self.n_bands)
        )
        sin_feat = (
            sin_pl.unsqueeze(1)
            * mag_pl.unsqueeze(1)
...
class FeedForwardV2(nn.Module):
    ...
    def __init__(
        ...
        if hidden_act not in act_map:
            raise ValueError(f"Unsupported hidden_act '{hidden_act}' in pswrecv2.")
...
class PSWBlockV2(nn.Module):
    """Single transformer-style layer: PhaseSyncAttentionV2 + FFN."""
    ...
    def __init__(
        ...
        super().__init__()
        self.attn = PhaseSyncAttentionV2(
            n_heads=n_heads,
...
class PSWEncoderV2(nn.Module):
    """Stack of PSWBlockV2 layers."""
...
class pswrecv2(SequentialRecommender):
    r"""pswrecv2: Magnitude-gated, Gabor-initialized PSWRec variant."""

    def __init__(self, config, dataset):
        super().__init__(config, dataset)

        # Core transformer hyperparameters (keep hidden_size=64).
        ...
        self.band_kernel_sizes = band_kernel_sizes
        self.band_dilations = band_dilations
        ...
        self.phase_filter = LocalPhaseFilterBankV2(
            hidden_size=self.hidden_size,
            kernel_sizes=band_kernel_sizes,
            dilations=band_dilations,
        )
        self.encoder = PSWEncoderV2(
        ...
        # Gabor-style init for filterbank.
    ...
    def _init_gabor_weights(self):
        """Initialize filterbank convs with simple Gabor-like kernels."""
        for i, (k, d) in enumerate(
            zip(self.band_kernel_sizes, self.band_dilations)
        ):
            # Band-specific frequency (higher index → higher freq).
            freq = (i + 1) / max(self.n_bands, 1)
            t = torch.arange(k, dtype=torch.float32) - (k - 1) / 2.0

            sigma = k / 4.0
            gauss = torch.exp(-t.pow(2) / (2.0 * sigma * sigma))

            cos_kernel = torch.cos(2.0 * math.pi * freq * t) * gauss
            sin_kernel = torch.sin(2.0 * math.pi * freq * t) * gauss

            # Expand to [hidden_size, 1, kernel_size] for depthwise conv.
            cos_kernel = (
                cos_kernel.view(1, 1, -1)
                .repeat(self.hidden_size, 1, 1)
                .contiguous()
            )
            sin_kernel = (
                sin_kernel.view(1, 1, -1)
                .repeat(self.hidden_size, 1, 1)
                .contiguous()
            )

            # Slightly scale down to avoid overly large initial activations.
            scale = 0.1
            self.phase_filter.real_convs[i].weight.data.copy_(cos_kernel * scale)
            self.phase_filter.imag_convs[i].weight.data.copy_(sin_kernel * scale)
    ...
    def forward(self, item_seq: torch.Tensor, item_seq_len: torch.Tensor) -> torch.Tensor:
        ...
        # Optional phase smoothness regularization.
        if self.phase_aux:
