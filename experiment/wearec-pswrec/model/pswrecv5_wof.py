...
"""
PSWRecV5 adapted for the WEARec Official Framework (WOF).

This is a self-contained copy of the PSWRecV5 architecture that conforms to
WEARec's ``SequentialRecModel`` base class interface so that it can be trained
and evaluated through WEARec's data pipeline, trainer, and metrics -- enabling
an apples-to-apples comparison on the same playground.

Architecture is *identical* to the RecBole version
(RecBole/recbole/model/sequential_recommender/pswrecv5.py).
Only the outer model shell (embeddings, forward/loss signatures) is adapted.
"""
...
# ---------------------------------------------------------------------------
# Architecture components (verbatim from RecBole pswrecv5.py)
# ---------------------------------------------------------------------------
...
class PhaseSyncAttentionV5(nn.Module):
    ...
    def forward(
        ...
        # Harmonic-mean magnitude gating (same as V4H)
        ...
        # V5: post-softmax phase gating and renormalize
...
class FeedForwardV5(nn.Module):
    ...
    def __init__(
        ...
        if hidden_act not in act_map:
            raise ValueError(f"Unsupported hidden_act '{hidden_act}' in pswrecv5_wof.")
...
class PSWBlockV5(nn.Module):
    """Single transformer-style layer: PhaseSyncAttentionV5 + FFN."""
    ...
    def __init__(
        ...
        super().__init__()
        self.attn = PhaseSyncAttentionV5(
            n_heads=n_heads,
...
class PSWEncoderV5(nn.Module):
    """Stack of PSWBlockV5 layers."""
...
# ---------------------------------------------------------------------------
# WOF model shell -- adapts PSWRecV5 to WEARec's SequentialRecModel interface
# ---------------------------------------------------------------------------

class PSWRecV5WOFModel(SequentialRecModel):
    r"""PSWRecV5 on WEARec Official Framework.

    Architecture is identical to the RecBole ``pswrecv5``.  Only the outer
    shell (embeddings, forward / loss / predict signatures) is adapted to
    WEARec's ``SequentialRecModel`` base class so that it plugs into
    WEARec's trainer, data-loader, and evaluation.
    """

    def __init__(self, args):
        super().__init__(args)

        # ---- map WEARec args to PSWRecV5 params ----
        ...
        # PSWRecV5-specific hyperparameters (passed via extra CLI args)
