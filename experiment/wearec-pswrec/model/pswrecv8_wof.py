...
"""
PSWRecV8 adapted for the WEARec Official Framework (WOF).

V8 = V5 + **Dynamic Phase Evolution (DPE)**
        + **Cross-Frequency Phase Coupling (CFPC)**
        + **Phase-Aware Values (PAV)**.

On top of V7, V8 adds phase-aware modulation of the value pathway.
In V5-V7, phase only affects *where* to attend (the attention weights).
V8 also lets phase influence *what* gets extracted: the attention-value
product is element-wise scaled by a phase-derived weight, so that value
vectors from phase-coherent positions are amplified and incoherent ones
are suppressed.
"""
...
# ---------------------------------------------------------------------------
# FilterBank (same as V5-V7)
# ---------------------------------------------------------------------------

class LocalPhaseFilterBankV8(nn.Module):
...
# ---------------------------------------------------------------------------
# V8 CHANGE: PhaseSyncAttentionV8 -- CFPC + Phase-Aware Values
# ---------------------------------------------------------------------------

class PhaseSyncAttentionV8(nn.Module):
    r"""Multi-head self-attention with phase bias, post-softmax gating,
    Cross-Frequency Phase Coupling (CFPC), and **Phase-Aware Values (PAV)**.

    PAV modulates the attention-value product by phase coherence:
      context = matmul(attn_probs_gated * phase_value_weight, v)
    where phase_value_weight = sigmoid(phase_value_scale * phase_scores).
    ...
    def __init__(
        ...
        # --- V7: cross-frequency band coupling ---
        self.band_coupling = nn.Parameter(
            torch.eye(n_bands).unsqueeze(0).repeat(n_heads, 1, 1) * 3.0
        )

        # --- V8 NEW: phase-aware value scale ---
        # Learnable scalar; initialised small so PAV starts as a gentle modulation.
        self.phase_value_scale = nn.Parameter(
    ...
    def forward(
        ...
        # Post-softmax phase gating (from V5)
        ...
        # --- V8 PAV: phase-aware value modulation ---
        # phase_value_weight: [B, H, L, L] -- how much to amplify each value
        # based on phase coherence between query (row) and key (col) positions.
        phase_value_weight = torch.sigmoid(
            self.phase_value_scale * phase_scores
        )
        # Fold into the attention-value product
        context = torch.matmul(attn_probs_gated * phase_value_weight, v)
...
class FeedForwardV8(nn.Module):
    ...
    def __init__(
        ...
        if hidden_act not in act_map:
            raise ValueError(f"Unsupported hidden_act '{hidden_act}' in pswrecv8_wof.")
...
class PSWBlockV8(nn.Module):
    """Single transformer-style layer: PhaseSyncAttentionV8 + FFN."""
    ...
    def __init__(
        ...
        super().__init__()
        self.attn = PhaseSyncAttentionV8(
            n_heads=n_heads,
...
# ---------------------------------------------------------------------------
# Encoder with DPE (same structure as V6/V7)
# ---------------------------------------------------------------------------

class PSWEncoderV8(nn.Module):
    """Stack of PSWBlockV8 layers with Dynamic Phase Evolution."""
    ...
    def forward(
        ...
        hidden_states: torch.Tensor,
        attention_mask: torch.Tensor,
        phase_filter: "LocalPhaseFilterBankV8",
...
class PSWRecV8WOFModel(SequentialRecModel):
    r"""PSWRecV8 on WEARec Official Framework.
    ...
    def __init__(self, args):
        ...
        self.phase_filter = LocalPhaseFilterBankV8(
            hidden_size=self.hidden_size,
            kernel_sizes=band_kernel_sizes,
            dilations=band_dilations,
        )

        self.encoder = PSWEncoderV8(
