...
"""
PSWRecV7 adapted for the WEARec Official Framework (WOF).

V7 = V5 + **Dynamic Phase Evolution (DPE)** + **Cross-Frequency Phase Coupling (CFPC)**.

On top of V6's per-layer phase recomputation, V7 adds a learnable cross-band
coupling matrix to the attention module.  Instead of combining bands
independently (weighted sum), CFPC lets each band's phase features be
modulated by coherence with other bands.  This enables the model to detect
when a high-frequency "burst" is aligned with a low-frequency "habit" --
a far richer denoising mechanism than independent band weighting.
"""
...
# ---------------------------------------------------------------------------
# Architecture components -- FilterBank and FFN are same as V6
# ---------------------------------------------------------------------------

class LocalPhaseFilterBankV7(nn.Module):
    r"""Multi-band quadrature filterbank over the sequence axis.  (Same as V5/V6.)"""
...
# ---------------------------------------------------------------------------
# V7 CHANGE: PhaseSyncAttentionV7 -- adds Cross-Frequency Phase Coupling
# ---------------------------------------------------------------------------

class PhaseSyncAttentionV7(nn.Module):
    r"""Multi-head self-attention with phase bias, post-softmax gating,
    and **Cross-Frequency Phase Coupling (CFPC)**.

    CFPC replaces the independent band weighting with a learned cross-band
    coupling matrix.  Each band's phase features are modulated by a weighted
    combination of all bands' phases, enabling the model to detect cross-
    frequency phase coherence (e.g., burst aligned with habit).
    """
    ...
    def __init__(
        ...
        # --- V5/V6 band_logits (kept for magnitude weighting) ---
        ...
        # --- V7 NEW: cross-frequency band coupling ---
        # Learnable [H, n_bands, n_bands] matrix; softmax over last dim
        # gives the mixing weights for each output band from all input bands.
        # Initialised near identity so V7 starts close to V6 behaviour.
        self.band_coupling = nn.Parameter(
            torch.eye(n_bands).unsqueeze(0).repeat(n_heads, 1, 1) * 3.0
        )  # [H, S, S]  -- *3.0 so softmax is peaked near identity initially
    ...
    def forward(
        ...
        # cos_phi, sin_phi: [B, n_bands, L]
        cos_pl = cos_phi.permute(0, 2, 1)   # [B, L, n_bands]
        sin_pl = sin_phi.permute(0, 2, 1)   # [B, L, n_bands]
        mag_pl = mag.permute(0, 2, 1)       # [B, L, n_bands]

        # --- V7 CFPC: cross-band coupling ---
        # coupling_weights: [H, S_out, S_in]  (softmax over S_in)
        coupling_weights = torch.softmax(self.band_coupling, dim=-1)

        # Mix bands:  [B, L, S_in] x [H, S_out, S_in]^T -> [B, H, L, S_out]
        # einsum: 'bls, hts -> bhlt'  (s = S_in, t = S_out)
        cos_coupled = torch.einsum('bls,hts->bhlt', cos_pl, coupling_weights)
        sin_coupled = torch.einsum('bls,hts->bhlt', sin_pl, coupling_weights)

        # Phase scores from coupled features  [B, H, L, S] x [B, H, S, L] -> [B, H, L, L]
        phase_scores = torch.matmul(
            cos_coupled, cos_coupled.transpose(-1, -2)
        ) + torch.matmul(sin_coupled, sin_coupled.transpose(-1, -2))

        # Magnitude weighting (unchanged from V5/V6 -- uses band_logits)
...
class FeedForwardV7(nn.Module):
    """Position-wise feed-forward with residual + LayerNorm.  (Same as V5/V6.)"""
    ...
    def __init__(
        ...
        if hidden_act not in act_map:
            raise ValueError(f"Unsupported hidden_act '{hidden_act}' in pswrecv7_wof.")
...
class PSWBlockV7(nn.Module):
    """Single transformer-style layer: PhaseSyncAttentionV7 + FFN."""
    ...
    def __init__(
        ...
        super().__init__()
        self.attn = PhaseSyncAttentionV7(
            n_heads=n_heads,
...
# ---------------------------------------------------------------------------
# Encoder with DPE (same as V6)
# ---------------------------------------------------------------------------

class PSWEncoderV7(nn.Module):
    """Stack of PSWBlockV7 layers with Dynamic Phase Evolution."""
    ...
    def forward(
        ...
        hidden_states: torch.Tensor,
        attention_mask: torch.Tensor,
        phase_filter: "LocalPhaseFilterBankV7",
...
class PSWRecV7WOFModel(SequentialRecModel):
    r"""PSWRecV7 on WEARec Official Framework.
    ...
    def __init__(self, args):
        ...
        self.phase_filter = LocalPhaseFilterBankV7(
            hidden_size=self.hidden_size,
            kernel_sizes=band_kernel_sizes,
            dilations=band_dilations,
        )

        self.encoder = PSWEncoderV7(
