...
"""
PSWRecV11 adapted for the WEARec Official Framework (WOF).

This version implements Behavioral Rotary Attention (B-RoPE), which:
  - Rotates Q and K using behavioral phase φ from the filterbank
  - Leaves V untouched (preserves semantic payload)
  - Embeds cos(Δφ) directly into the attention geometry

Backbone (filterbank + FFN) matches PSWRecV5; only the attention mechanism differs.
"""
...
# ---------------------------------------------------------------------------
# Architecture components (filterbank shared with V5/V10)
# ---------------------------------------------------------------------------
...
class BehavioralRotaryAttentionV11(nn.Module):
    """Behavioral Rotary Attention (B-RoPE) for WOF.

    Rotates Q and K using phase φ; leaves V unchanged.
    """
...
class FeedForwardV11(nn.Module):
    ...
    def __init__(
        ...
        if hidden_act not in act_map:
            raise ValueError(f"Unsupported hidden_act '{hidden_act}' in pswrecv11_wof.")
...
# ---------------------------------------------------------------------------
# WOF model shell -- adapts PSWRecV11 to WEARec's SequentialRecModel interface
# ---------------------------------------------------------------------------


class PSWRecV11WOFModel(SequentialRecModel):
    r"""PSWRecV11 on WEARec Official Framework.

    Backbone as PSWRecV5, attention replaced by Behavioral Rotary Attention.
    """

    def __init__(self, args):
        super().__init__(args)

        # ---- map WEARec args to PSWRecV11 params ----
        ...
        # PSWRecV11-specific hyperparameters (passed via extra CLI args)
    ...
    def forward(self, input_ids, user_ids=None, all_sequence_output=False):
        ...
        # Standard Transformer embedding regularisation
        sequence_emb = self.LayerNorm(sequence_emb)
        sequence_emb = self.dropout(sequence_emb)

        # Phase filterbank on normalised embeddings
        cos_phi, sin_phi, mag = self.phase_filter(sequence_emb)

        # Optional phase-smoothness auxiliary regularisation
        if self.phase_aux:
